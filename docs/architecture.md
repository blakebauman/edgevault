...
| `delivery` (`delivery.edgevault.io`) | Data plane — serves **pre-resolved** configs/flags and **pre-rendered content pages** (`/v1/pages/:key`) from KV with an in-memory L1, gated by environment-scoped API keys. No business logic; cannot decrypt secrets. |
...
- **`VaultDurableObject`** — one SQLite DO per workspace (system of record):
  environments, config content, flags, content documents, revisions, promotions,
  activity log, and envelope-encrypted secret ciphertext. Hosts hibernatable
  WebSocket + SSE for live updates. Strong consistency per workspace, 10 GB ceiling.
...
| Config content, revisions, promotions, flags, content documents, activity log, **secret ciphertext** | Vault DO SQLite | strong per workspace |
| Pre-resolved edge values `config:{ws}:{env}:{key}` + pre-rendered content HTML `html:{ws}:{env}:{key}` | KV (cache-tagged) | eventual (write-through) |
